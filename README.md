...
`admin.html` 을 브라우저로 열고 비밀번호를 넣습니다. 서버가 없으므로
이 화면은 **DB 를 고치는 것이 아니라 파일을 만들어 주는 도구**입니다.

⚠️ 비밀번호는 브라우저에서만 확인합니다 — 소스를 보면 우회됩니다.
진짜로 막으려면 Vercel 의 Deployment Protection 을 `/admin.html` 에 거세요.

1. 상품을 넣고 고칩니다 (작업 중인 내용은 브라우저에 남습니다)
2. **파일 내보내기** → `products.js` 를 받습니다
3. 받은 파일로 `js/data/products.js` 를 덮어쓰고 커밋·푸시합니다

⚠️ **3번까지 해야 사이트에 반영됩니다.** 2번을 안 하면 상품을 쉰 개
넣어 놓고 사이트는 그대로인 일이 생깁니다. 내보내지 않은 변경이 있으면
화면 위에 띠가 뜨고, 창을 닫으려 하면 붙잡습니다.

사업자 정보도 같은 방식입니다 (`사업자 정보` 탭 → `site.js`).

관리자가 막아 주는 것 — 전부 실제로 겪은 사고입니다:
- **id 중복** — 뒤엣것이 영영 안 열립니다
- **분류가 categories.js 에 없음** — 그 상품은 어느 목록에도 안 뜹니다
- **사진 이름이 틀림** — 카드가 빈 상자가 됩니다 (실제로 파일을 불러 확인합니다)
- **후기 수를 적어 둠** — 없는 후기를 있다고 적는 셈입니다 (경고)
